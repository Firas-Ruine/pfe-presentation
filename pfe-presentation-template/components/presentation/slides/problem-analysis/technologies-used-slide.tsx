...
const technologies = [
  { 
    ...
    icon: Layers, 
    items: [
      { name: "LangGraph", logo: "https://avatars.githubusercontent.com/u/126733545?s=40", desc: "Workflow" },
      { name: "LangChain", logo: "https://avatars.githubusercontent.com/u/126733545?s=40", desc: "LLM tools" },
      { name: "LangSmith", logo: "https://avatars.githubusercontent.com/u/126733545?s=40", desc: "Observability" },
    ]
  },
  { 
    category: "AI & ML", 
            ...
            <Card key={index} className="shadow-md">
              <CardContent className="p-4">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <span className="font-bold text-xl">{category.category}</span>
                </div>
                <div className="space-y-2">
                  {category.items.map((item, idx) => (
                    <div key={idx} className="flex items-center gap-3 p-2 bg-muted/30 rounded-lg">
                      <div className="w-10 h-10 flex-shrink-0 flex items-center justify-center bg-white rounded-lg p-1.5">
                        <img src={item.logo} alt={item.name} className="w-full h-full object-contain"
                          onError={(e) => { (e.target as HTMLImageElement).style.display = 'none'; }} />
                      </div>
                      <div className="flex-1 min-w-0">
                        <span className="font-semibold text-lg block leading-tight">{item.name}</span>
                ...
                <div className="grid grid-cols-2 gap-2">
                  {category.items.map((item, idx) => (
                    <div key={idx} className="flex items-center gap-2 p-2 bg-muted/30 rounded-lg">
                      <div className="w-9 h-9 flex-shrink-0 flex items-center justify-center bg-white rounded-lg p-1">
                        <img src={item.logo} alt={item.name} className="w-full h-full object-contain"
                          onError={(e) => { (e.target as HTMLImageElement).style.display = 'none'; }} />
                      </div>
                      <div className="flex-1 min-w-0">
